  ...
  imports: [
    PinoLoggerModule.forRoot({
      pinoHttp: {
        ...
        customLogLevel: (res: IncomingMessage, _, err: Error) => {
        ...
        customSuccessMessage: function (res: IncomingMessage) {
          return res.statusCode >= 400 && res.statusCode < 500
            ? `${res.statusCode} ${statusText[res.statusCode]} `
            : `${res.statusCode} ${statusText[res.statusCode]}`.trim();
        },
        customErrorMessage: function (_, res: ServerResponse, error: Error) {
